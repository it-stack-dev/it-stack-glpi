...
docker compose logs --tail=50 glpi
...
1. Check logs: docker compose logs glpi
2. Verify environment variables are set correctly
3. Check database connectivity: pg_isready -h lab-db1 -p 5432
4. Verify port is not already in use: ss -tlnp | grep 80
...
```bash
# Test connectivity
psql -h lab-db1 -U glpi_user -d glpi_db -c '\conninfo'

# Check pg_hba.conf allows connection from lab-mgmt1
```

### Performance issues

1. Check resource usage: docker stats it-stack-glpi
...
| Log | Path |
|-----|------|
| Application | docker compose logs glpi |
